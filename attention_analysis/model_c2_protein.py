...
class Predictor(nn.Module):
    ...
    def gcn(self, input, adj):
        ...
        input.to(device)
        #input = input.cuda()
        #self.weight_1 = self.weight_1.cuda()
    ...
    def __call__(self, data, train=True):
        ...
        if train:
            ...
            predicted_interaction,attention = self.forward(compound, adj, protein,atom_num,protein_num)
            correct_labels = correct_interaction.to('cpu').data.numpy()
            ys = F.softmax(predicted_interaction, 1).to('cpu').data.numpy()
            predicted_labels = np.argmax(ys, axis=1)
            predicted_scores = ys[:, 1]
            return correct_labels,predicted_labels,predicted_scores,attention
