...
class Predictor(nn.Module):
    ...
    def forward(self, compound, adj,  protein,atom_num,protein_num):
        ...
        out,norm,trg,sum = self.decoder(compound, enc_src, compound_mask, protein_mask)
        # out = [batch size, 2]
    ...
    def __call__(self, data, train=True):
        ...
        if train:
            ...
            predicted_interaction,norm,trg,sum = self.forward(compound, adj, protein,atom_num,protein_num)
            correct_labels = correct_interaction.to('cpu').data.numpy()
            ys = F.softmax(predicted_interaction, 1).to('cpu').data.numpy()
            predicted_labels = np.argmax(ys, axis=1)
            predicted_scores = ys[:, 1]
            return correct_labels, predicted_labels, predicted_scores,norm,trg,sum
    ...
    def valid(self, dataset):
        ...
        with torch.no_grad():
            for data in dataset:
                ...
                labels.append(label)
                data = pack(atoms,adjs,proteins, labels, self.model.device)
                loss_dev = self.model(data, train=True)
                loss_total_dev += loss_dev
        return loss_total_dev
"""
